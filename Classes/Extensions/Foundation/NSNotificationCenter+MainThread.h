...
+ (id <NSObject>)addObserverOnMainThreadForName:(NSString *)name object:(nullable id)object usingBlock:(void (^)(NSNotification *note))block NS_SWIFT_NAME(addObserverOnMainThread(forName:object:handler:));
+ (id <NSObject>)addObserverOnMainThreadForName:(NSString *)name usingBlock:(void (^)(NSNotification *note))block NS_SWIFT_NAME(addObserverOnMainThread(forName:handler:));
